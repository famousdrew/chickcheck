...
import { findFlockById, calculateCurrentWeek } from "@/lib/services/flocks";
import {
  findChickById,
  createChickPhoto,
  findPhotosByChickId,
...
export const POST = withErrorHandler(
  async (
    ...
    try {
      ...
      // Calculate week number using Pacific timezone
      let weekNumber: number | undefined;
      if (flock.startDate) {
        weekNumber = calculateCurrentWeek(flock.startDate);
      }
      ...
      throw error; // Let withErrorHandler catch unexpected errors
    }
  }
);
